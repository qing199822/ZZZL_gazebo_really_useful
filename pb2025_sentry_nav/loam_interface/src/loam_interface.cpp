...
: Node("loam_interface", options)
{
  ...
  // base_frame_to_lidar_initialized_ now class-initialized (L1)
  tf_buffer_ = std::make_unique<tf2_ros::Buffer>(this->get_clock());
  tf_listener_ = std::make_unique<tf2_ros::TransformListener>(*tf_buffer_);

  // L3 TODO: parameterize output topics to avoid potential feedback loops with input topics
...
void LoamInterfaceNode::pointCloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  ...
  // CRITICAL FIX: check initialization before using tf_odom_to_lidar_odom_
  {
    std::lock_guard<std::mutex> lock(state_mutex_);
    if (!base_frame_to_lidar_initialized_) {
      RCLCPP_WARN(this->get_logger(),
        "Dropping point cloud: TF not yet initialized (waiting for odometry callback)");
      return;
    }
  }
...
void LoamInterfaceNode::odometryCallback(const nav_msgs::msg::Odometry::ConstSharedPtr msg)
{
  RCLCPP_INFO_ONCE(this->get_logger(),
    "odometryCallback FIRST CALL - msg frame_id=%s, stamp=%d.%09d",
  ...
  // Here we transform it to the `odom` frame
  {
    ...
    if (!base_frame_to_lidar_initialized_) {
      RCLCPP_INFO(this->get_logger(),
        "TF init: looking up %s -> %s at time %d.%09d",
        base_frame_.c_str(), lidar_frame_.c_str(),
        msg->header.stamp.sec, msg->header.stamp.nanosec);
      try {
        auto tf_stamped = tf_buffer_->lookupTransform(
          base_frame_, lidar_frame_, rclcpp::Time(0), rclcpp::Duration::from_seconds(0.5));
        ...
        RCLCPP_INFO(this->get_logger(),
          "TF init SUCCESS: %s->%s = [%.3f, %.3f, %.3f]",
          base_frame_.c_str(), lidar_frame_.c_str(),
          tf_base_frame_to_lidar.getOrigin().x(),
          tf_base_frame_to_lidar.getOrigin().y(),
          tf_base_frame_to_lidar.getOrigin().z());
      } catch (tf2::TransformException & ex) {
        RCLCPP_WARN(this->get_logger(),
          "TF lookup %s->%s failed: %s Retrying...",
  ...
  RCLCPP_INFO_ONCE(this->get_logger(),
    "Publishing lidar_odometry: frame=%s, child=%s, pos=[%.3f,%.3f,%.3f]",
    out.header.frame_id.c_str(), out.child_frame_id.c_str(),
