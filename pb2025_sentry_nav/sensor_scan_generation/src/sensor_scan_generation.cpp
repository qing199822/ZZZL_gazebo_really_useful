  ...
  const sensor_msgs::msg::PointCloud2::ConstSharedPtr & pcd_msg)
{
  ...
  // H3 FIX: forward upstream velocity from point_lio instead of noisy finite differences
  publishOdometry(
    tf_odom_to_robot_base, odometry_msg->header.frame_id, robot_base_frame_,
    pcd_msg->header.stamp, &odometry_msg->twist.twist);
  ...
  const std::string & target_frame, const std::string & source_frame, const rclcpp::Time & time)
{
  try {
    ...
    RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), 5000,
      "TF lookup %s->%s failed: %s. Using identity fallback - navigation may drift!",
...
void SensorScanGenerationNode::publishOdometry(
  ...
  const rclcpp::Time & stamp, const geometry_msgs::msg::Twist * upstream_twist)
{
  ...
  // H3 FIX: prefer upstream velocity estimate from point_lio over finite differences
  if (upstream_twist) {
    out.twist.twist = *upstream_twist;
  } else {
    // Fallback: finite-difference velocity (only when upstream unavailable)
    std::lock_guard<std::mutex> lock(velocity_mutex_);

    if (velocity_initialized_) {
      const double dt = (stamp - previous_time_ros_).seconds();
